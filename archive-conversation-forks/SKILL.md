...
- Canonical of a fork-family theme -> prefix with **`[main]`** at the very front, then the label + what the whole thread is/did. (`[main]` pairs with `[scroll-dep]` so the family's primary and its kept scrollback segments are both flagged even when the picker truncates.) **A distinct sub-lineage head also gets `[main]`** - it is not one-per-family, and which sessions qualify is decided by **measurement, never by issue number, branch, or a one-`[main]`-per-family assumption**. The test: a head qualifies when its prose is **near-disjoint** from the other heads (low `fps_prose` overlap); a session that mostly overlaps an existing head is a *downstream fork* of it and gets no marker. (Real miss: an 1887 session 99% disjoint from the 2110 head was left unmarked because `[main]` was assigned one-per-family by assumption, while a 2268 session 82% contained in the 2110 head correctly got none - the 82%-overlap session is the fork, the 1%-overlap one is the peer head.) e.g. within a `claude-patches` family, both the empty-fork-diagnosis canonical and the patch-development head are `[main]` of their respective sub-lineages. A retained fork with only modest unique content -> label + what is unique to it (no marker).
