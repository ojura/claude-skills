...
Every failure path (CDP error, stalled walk, fragmented ranges, decrypt failures, expired media)
aborts loudly or is marked visibly in the artifact, rather than producing a confident partial.
...
Usage:
  ...
  tz       - an IANA timezone for rendered timestamps (e.g. Europe/Zagreb). Omit for machine local.
...
Outputs (written 0600 in an OUT dir created 0700, because they are decrypted private data):
  index.html       - self-contained transcript + media
  media/           - downloaded photos/files/videos
  messages.json    - the decrypted rows {mid, sender, ts, text, status}, raw plaintext
"""
...
if len(sys.argv) > 6 and sys.argv[6]:
    ...
    TZ = datetime.datetime.now().astimezone().tzinfo; TZNAME = str(TZ)
...
# --- daemon health check (translate the most common operator mistake into a hint) ---
try:
    get("/status", 5)
except Exception:
...
# Thread is "ready" when its LightSpeed runtime is up AND a range row for TK is
# materialized in this tab. (An innerText heuristic misses loaded-but-backgrounded
# restored tabs.) Activate the tab first so a discarded/restored tab actually loads.
def ls_ready(sid):
...
def wait_ls(sid, tries=20):
    ...
    return False
def thread_page():
    pat = re.compile(r"/t/%s(?:[/?#]|$)" % re.escape(TK))   # path-boundary match, not bare substring
    for tgt in [t for t in get("/targets") if t["type"] == "page" and pat.search(t.get("url", ""))]:
...
for direction, flag, load, ref, pick in [(0, "hasMoreBefore", "isLoadingBefore", "minTimestampMs", "min"),
    ...
    for chunk in range(60):
        ...
        if s.get("norow"):
            print("  %s chunk%d: no range row yet" % (dirn, chunk)); time.sleep(2); continue
...
def dec(t):
    """Return (status, text). status: 'dec' decrypted | 'plain' passthrough (no marker) | 'fail' decrypt error."""
    ...
    x = t[len(PAS):-len(PAS)]
    try:
        raw = base64.b64decode(x + "=" * (-len(x) % 4), validate=True)   # exact padding per residue; strict catches a stray byte (the "+===" over-pad breaks strict mode)
...
for start in range(0, n, 2500):
    rows += json.loads(ev(
        "(function(s,e){var I64=require('I64');return JSON.stringify(window.__dm.slice(s,e).map(function(m){"
        "return {mid:m.messageId,sender:I64.to_string(m.senderId),ts:I64.to_string(m.timestampMs),"
        "text:(typeof m.text==='string'?m.text:'')};}));})(%d,%d)" % (start, start + 2500), PGS, 60))

dec_ok = plain = fail = wrapped = 0
for r in rows:
    if r.get("text") and r["text"].startswith(PAS): wrapped += 1
    st, txt = dec(r.get("text"))
    r["text"] = txt; r["status"] = st
    if st == "dec": dec_ok += 1
    elif st == "plain": plain += 1
    else: fail += 1
print("[decrypt] decrypted=%d plaintext-passthrough=%d failed=%d (wrapped=%d)" % (dec_ok, plain, fail, wrapped))

# E2EE / wrong-key guard: refuse to emit a confident garbage artifact (fail loud, not silent).
if wrapped == 0 and n >= 20:
    die("no vault-encrypted messages found in %d rows - this is almost certainly an E2EE 'secret' thread "
        "(or wrong DB). This tool cannot decrypt those. Output suppressed." % n)
if wrapped and fail > max(5, 0.02 * wrapped):
    die("high decrypt-failure rate %d/%d - likely the wrong key or an E2EE thread. Output suppressed." % (fail, wrapped))
...
try:
    NAMES = json.loads(ev(
        ...
        "var id=I64.to_string(c.id||c.contactId||c.profileFbid);var nme=c.name||c.firstName||c.displayName||(c.profile&&c.profile.name);"
...
L = json.loads(ev(linkjs, PGS, 90, await_promise=True) or "{}")
...
# --- 4. download media (validate: reject expired-CDN 1x1 placeholders, retry once) ---
os.makedirs(OUT, mode=0o700, exist_ok=True)
os.chmod(OUT, 0o700)
os.makedirs(OUT + "/media", mode=0o700, exist_ok=True)
os.chmod(OUT + "/media", 0o700)   # enforce on a pre-existing dir too (makedirs exist_ok doesn't re-chmod)
def ext(u, mime):
...
def is_placeholder(p):
    try:
        if os.path.getsize(p) < 250: return True
        with open(p, "rb") as f: head = f.read(24)
        if head[:8] == b"\x89PNG\r\n\x1a\n":
            w = int.from_bytes(head[16:20], "big"); h = int.from_bytes(head[20:24], "big")
            if w <= 1 and h <= 1: return True     # FB CDN returns a 1x1 PNG for expired/invalid URLs
    except Exception: pass
    return False
all_atts = [a for lst in media.values() for a in lst]
jobs = {a["url"]: OUT + "/media/" + hashlib.md5(a["url"].encode()).hexdigest()[:16] + "." + ext(a["url"], a.get("mime"))
        for a in all_atts}
def dl(it):
    u, p = it
    if os.path.exists(p) and not is_placeholder(p): return (u, p, 1)
    for _ in range(2):   # the fresh sync above reissues valid URLs; one retry covers a transient miss
        ...
        if os.path.exists(p) and os.path.getsize(p) > 0 and not is_placeholder(p): return (u, p, 1)
    return (u, p, 0)
loc = {}
...
got = len(loc)
print("[media] %d/%d attachments downloaded (failed/expired: %d)" % (got, att_total, att_total - got))
if got < att_total:
...
msgs = sorted([r for r in rows if r.get("ts") and str(r["ts"]).lstrip("-").isdigit()], key=lambda x: int(x["ts"]))
...
others = {m["sender"] for m in msgs if m["sender"] != SELF_ID}
is_group = len(others) > 1
if is_group: print("[render] group thread: %d non-self participants; resolving names from the contacts table" % len(others))
def who(sid):
...
for m in msgs:
    ...
    if m["mid"] in cards:
        ...
        if u:
            label = (c.get("title") or c.get("sub") or u)[:60]
            body += '<div class=card>🔗 <a href="%s" target=_blank rel=noopener>%s</a></div>' % (html.escape(u), html.escape(label))
