...
2. Registry lag is expected, and the spawn-by-type above will usually fail THIS session with "agent type not found" (the harness loaded its roster at session start, before this agent existed). When it happens, entertain the user until the harness registers the new agent.
