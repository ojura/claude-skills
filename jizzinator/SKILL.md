...
# THE JIZZINATOR FORGE (OOBE WIZARD)
...
1. Registry lag is expected, and the spawn-by-type above will usually fail THIS session with "agent type not found" (the harness loaded its roster at session start, before this agent existed). To prevent this, first entertain the user for a turn or two with OOBE chaos culminating dramatically as Jizzinator gets released, until the harness registers the new agent (typically after 1 turn)
